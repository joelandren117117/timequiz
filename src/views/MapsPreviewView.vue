<template>
    <div class="maps-preview">
      ...
      <!-- Karta 3: Admin Map (alla spelares svar) -->
      <section class="map-section">
  ...
  // Center ungefär över Europa
  const initialCenter = [54, 15];
  
  // State för spelarens aktuella gissning (karta 1)
  const playerGuess = ref(null);
  
  // Exempel på korrekt plats (Eiffeltornet)
  const correctLocation = ref({
    lat: 48.8584,
    lng: 2.2945,
  });
  
  // Dummy-spelare och deras gissningar (för admin-kartan)
  const players = ref([
    {
      id: 'p1',
      name: 'Alice',
    ...
    },
    {
      id: 'p2',
      name: 'Bob',
      guess: { lat: 51.50, lng: -0.12 },
    },
    {
      id: 'p3',
      name: 'Charlie',
      guess: { lat: 52.52, lng: 13.40 },
    },
  ]);
  
  // Markers för admin-kartan baserat på players
  ...
  // Hantera klick på "Player Guess Map"
  const onPlayerMapClick = ({ lat, lng }) => {
    playerGuess.value = { lat, lng };
  };
  
  // Skicka gissning (här bara console.log, senare Socket.io)
  const submitGuess = () => {
